...
def fetch_ratios(symbol):
    url = f"https://financialmodelingprep.com/api/v3/ratios/{symbol}?period=quarter&limit=1000&apikey={FMP_API_KEY}"
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(fetch_ratios, symbol) for symbol in symbols]
        for future in tqdm(as_completed(futures), total=len(symbols), desc="Processing symbols"):
            symbol, data = future.result()
            if data:
                df = pd.DataFrame(data)
                df.to_sql('equity_ratios', engine, if_exists='append', index=False)
            else:
