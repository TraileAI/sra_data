...
def compute_metrics(symbol, df_fund, df_bench, df_rf, current_date):
    ...
    ret_bench_d = df_bench_slice.loc[ret_fund_d.index].pct_change().dropna()
    ...
    if pd.isna(fund_std) or fund_std == 0:
        sharpe = 0.0
    else:
        sharpe = excess.mean() / fund_std * np.sqrt(252)
...
def main(benchmark='SPY'):
    ...
    for current_date in tqdm(dates, desc="Computing historical scores"):
        metrics_this_month = []
        with ThreadPoolExecutor(max_workers=10) as ex:
            futures = [ex.submit(compute_metrics, s, funds.get(s), df_bench, df_rf, current_date) for s in symbols]
            for future in as_completed(futures):
                m = future.result()
                if m:
                    metrics_this_month.append(m)
        if metrics_this_month:
            scores = calculate_scores(metrics_this_month)
            print(f"\n--- Scores as of {current_date.strftime('%Y-%m-%d')} ---")
